...
class PersonalSettingsGenderIdentityWidget extends StatelessWidget
    with ScreenUtils {
  const PersonalSettingsGenderIdentityWidget({super.key});
        ...
        return AppDropdownWidget<String>(
          onChanged: (p0) {
            context.read<PersonalSettingsCubit>().updateState(
                  userGenderIdentity: p0,
                );
          },
          labelText: LocaleKeys.personalSettings_genderIdentity.tr(),
          value: genderIndentityItemList.any(
            (p0) => p0.value == state.userGenderIdentity,
          )
              ? state.userGenderIdentity
              : null,
          items: genderIndentityItemList
